  ...
  >
    <el-form ref="addTenant" :rules="rules" :model="tenant" label-width="120px">
      ...
      <el-form-item label="封面" prop="banner">
        <upload :type="'image'" :limit="1" :image="tenant.banner" @upload="upload" ></upload>
      </el-form-item>
      <el-form-item label="头像" prop="cover">
        <upload :type="'image'" :limit="1" :image="tenant.cover" @upload="uploadCover" ></upload>
      </el-form-item>
    </el-form>
...
<script>
import {editTenant,queryTenant,checkTenantCode} from '@/api/tenantapi'
...
export default {
  ...
  methods: {
    open(){
      queryTenant(this.currentRow).then(resp=>{
        if(resp.code == 0){
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          editTenant(_this.tenant).then(resp=>{
            if(resp.code == 0){
